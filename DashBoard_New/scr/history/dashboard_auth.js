...
function updateUserBadge() {
	...
	if (currentUser) {
		badge.innerHTML =
			...
			`<button class="btn sm" style="padding:2px 6px" onclick="showAuditLogModal()">📋 Lịch sử</button> ` +
...
async function apiGetAuditLog() {
	try {
		const r = await fetch(`${API_BASE}?action=get_audit_log&limit=100`);
		return await r.json();
	} catch (e) {
		return { success: false, error: 'Không kết nối được server' };
	}
...
async function showAuditLogModal() {
	openModal('modal_auditlog');
	const body = document.getElementById('auditlog_body');
	if (body) body.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:10px;color:var(--t3)">⏳ Đang tải...</td></tr>';
	const res = await apiGetAuditLog();
	if (!body) return;
	if (!res.success) {
		body.innerHTML = `<tr><td colspan="3" style="text-align:center;padding:10px;color:var(--red-m)">❌ ${res.error || 'Lỗi tải dữ liệu'}</td></tr>`;
		return;
	}
	if (!res.data || !res.data.length) {
		body.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:10px;color:var(--t3)">Chưa có lịch sử nào</td></tr>';
		return;
	}
	body.innerHTML = res.data
		.map(
			(r) =>
				`<tr><td style="font-family:var(--mono);font-size:10px">${r.created_at}</td><td>${r.username || '—'}</td><td>${r.action}</td></tr>`,
		)
		.join('');
}
